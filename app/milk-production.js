...
export default function MilkProductionScreen() {
  ...
  const handleGetAIAnalysis = async () => {
    if (!cowData || !currentRecord) {
      Alert.alert('No Data', 'Please enter or view milk production data first.');
      return;
    }
    ...
    try {
      ...
      setAiAnalysis(result.analysis || result.predictions || 'No analysis available.');
  ...
  return (
    <SafeAreaView style={styles.container}>
      <ScrollView contentContainerStyle={styles.content}>
        ...
        {/* AI Analysis Button */}
        {hasAnyRecord && (mode === 'viewDetails' || mode === 'enterData') && (
          <TouchableOpacity 
