...
export default function HomeScreen() {
  ...
  const dashboardItems = [
    ...
    { id: 3, title: 'Daily Reports', subtitle: 'Track daily activities', icon: 'document-text', color: '#FF9800', route: '/daily-reports' },
    { id: 4, title: 'Milk Production', subtitle: 'Monitor milk yield', icon: 'water', color: '#9C27B0', route: '/milk-production' },
    { id: 5, title: 'Expenses', subtitle: 'Track farm expenses', icon: 'calculator', color: '#F44336', route: '/expenses' },
    { id: 6, title: 'Reports', subtitle: 'View analytics & reports', icon: 'bar-chart', color: '#607D8B', route: '/reports' },
  ];
        ...
        <ScrollView style={styles.dashboard} showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.grid}>
            {dashboardItems.map((item, index) => (
              <Animated.View key={item.id} style={[styles.gridItem, { opacity: fadeAnim, transform: [{ translateY: slideAnim }] }]}>
                <TouchableOpacity style={[styles.itemCard, { borderLeftColor: item.color }]} onPress={() => handleItemPress(item.route)}>
...
const styles = StyleSheet.create({
  ...
  greeting: { fontSize: 16, color: '#7f8c8d', marginBottom: 4 },
  farmName: { fontSize: 24, fontWeight: 'bold', color: '#2c3e50' },
  logoutButton: { padding: 8, borderRadius: 8, backgroundColor: '#fff5f5' },
  ...
  itemSubtitle: { fontSize: 12, color: '#7f8c8d', lineHeight: 16 },
});
